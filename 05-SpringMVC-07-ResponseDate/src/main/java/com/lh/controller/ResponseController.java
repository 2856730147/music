...
public class ResponseController {
    ...
    public void testVoid() {
        System.out.println("测试返回值void,无法被视图解析了");
    }

    @RequestMapping("testString")
    public String testString() {
        System.out.println("测试返回值String,直接进行跳转");
        return "/success.jsp";
    }
...
//        设置模型
        model.addAttribute("username", "马克马克");
        //request.setAttribute("username", "易烊千玺);
    ...
    public void testVoidForward(HttpServletRequest request, HttpServletResponse response) {
        request.setAttribute("username", "马克马克");
    ...
    /**
     * testResponseBody注解会自动跳过视图解析器，并且返回json类的数据
     */
    ...
    public String testHtmlString() {
        /**
         *如果直接使用字符串返回就相当于调用，resp.getWriter().write("/success.jsp")
         **/
